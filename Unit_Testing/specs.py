...
def count_sets(cards):
    ...
    """
    if len(cards) != 12:
        raise ValueError("exactly 12 cards required")
    for c in cards:
        if len(c) != 4:
            raise ValueError("cards must have exactly 4 digits")
        for i in range(4):
            if c[i] != "0" and c[i] != "1" and c[i] != "2":
                raise ValueError("cards must be integers in base 3")
    combos = set(combinations(cards, 3))
    num_sets = 0
    if len(combos) != 220:
        raise ValueError("cards not unique")
    for cmb in combos:
        if is_set(cmb[0], cmb[1], cmb[2]):
            num_sets += 1
            print(cmb)
    return num_sets
...
def is_set(a, b, c):
    ...
    isset = True
    for i in range(4):
        sum_i = int(a[i])+int(b[i])+int(c[i])
        if sum_i != 0 and sum_i != 3 and sum_i != 6:
            isset = False
    return isset
    #raise NotImplementedError("Problem 6 Incomplete")

"""if __name__ == "__main__":
    #print(smallest_factor(4))
    ...
    print(count_sets(no_sets))
    print(count_sets(one_set))
    print(count_sets(three_sets))
    print(count_sets(six_sets"""